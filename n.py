...
# Добавление таблицы для модераторов (добавить в init_db)
cursor.execute('''
...
async def add_moderator(message: Message):
    args = message.text.split()
    if len(args) < 2:
        await message.answer("❌ Использование: /admod <ID пользователя>")
        return
    ...
    try:
        ...
        if cursor.fetchone():
            await message.answer("❌ Этот пользователь уже модератор.")
        else:
            cursor.execute(
                'INSERT INTO moderators (user_id, added_by) VALUES (?, ?)',
                (user_id, message.from_user.id)
            )
            conn.commit()
...
async def delete_moderator(message: Message):
    args = message.text.split(maxsplit=2)
    if len(args) < 3:
        await message.answer("❌ Использование: /delmod <ID пользователя> <причина>")
        return
    ...
    try:
        ...
        if cursor.rowcount > 0:
            conn.commit()
            await message.answer(f"✅ Пользователь {user_id} удален из модераторов.
Причина: {reason}")
        else:
